...
def test_add_task():
    response = client.post("/tasks", params={"task": "Study DevOps"})
...
def test_get_tasks():
    client.post("/tasks", params={"task": "Learn FastAPI"})

    response = client.get("/tasks")

    assert response.status_code == 200
    assert response.json() == ["Learn FastAPI"]


def test_get_task_by_index():
    client.post("/tasks", params={"task": "Write tests"})
...
def test_delete_task():
    client.post("/tasks", params={"task": "Delete this task"})
